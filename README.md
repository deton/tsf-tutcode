...
| 鍵盤レイアウト                            | 仮想鍵盤上に表示する各文字の位置に対応するキーボードレイアウト<br>右手と左手ブロックの間は全角縦線｜を想定しています。<br>行の区切りは`\n`<br>キーシーケンス内の各文字が指定された位置にあるとみなしてレイアウトします。<br>デフォルト値:`12345｜67890\nqwert｜yuiop\nasdfg｜hjkl;\nzxcvb｜nm,./` |
| 初期表示内容                              | 何もキーが入力されていない時に表示する仮想鍵盤の内容。<br>右手と左手ブロックの間は全角縦線｜を想定しています。<br>行の区切りは`\n`<br>TUT-Codeの例: `　　　　　｜　　　　　\nやまかあは｜歳度校銀転\nわさたなら｜化下回歴係\n若記委入実｜論値路装働` |
